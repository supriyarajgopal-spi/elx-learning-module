...
do {
  try {
    $cursor = elx_user_fav_cursor($database);
    // Store object ID of the last successfully processed record.
    $saved_id = NULL;
    while (!empty($cursor)) {
      elx_user_fav_batch($cursor, $database, $saved_id);
      $cursor = elx_user_fav_cursor($database, $saved_id);
    }
    $disconnected = FALSE;
  }
 ...
 */
function elx_user_fav_cursor(MongoDB\Database $database, MongoDB\BSON\ObjectID $saved_id = NULL) {
  if (isset($saved_id)) {
    $query = array(
  ...
  return $database->favorites->find($query, array(
    'sort' => array('_id' => 1),
    'limit' => 100,
  ));
}
 ...
 */
function elx_user_fav_batch(MongoDB\Driver\Cursor $cursor, MongoDB\Database $database, &$saved_id) {
  $col_count = 0;
